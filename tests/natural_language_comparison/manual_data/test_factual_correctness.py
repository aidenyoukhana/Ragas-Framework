"""
Test Factual Correctness - Evaluation with Metrics
"""
...
def test_factual_correctness_with_metrics(ragas_dataset):
    """Test Factual Correctness metric with actual evaluation and scoring."""
    ...
    for idx, sample in enumerate(ragas_dataset):
        ...
        reference = f"Sample reference answer {idx + 1}"  # Reference for factual correctness
    ...
    print(f"\n🔍 Running Factual Correctness evaluation...")
    
    try:
        result = evaluate(dataset, metrics=[FactualCorrectness()], llm=llm)
